    ...
    mDone(false)
{
    ...
    mPlayer = gcn::Image::load(ResourceHandler::getInstance()->getRealFilename("playerportrait.bmp"));
    mBoss = gcn::Image::load(ResourceHandler::getInstance()->getRealFilename("bossportrait.bmp"));
    mOlof = gcn::Image::load(ResourceHandler::getInstance()->getRealFilename("olofportrait.bmp"));
...
void Dialog::draw(gcn::Graphics *graphics)
{
    ...
    if (mCharacter == BOSS)
    {
        graphics->drawImage(mBoss, 5, 8);
    }
    else if (mCharacter == OLOF)
    {
        graphics->drawImage(mOlof, 5, 8);
    }
    else if (mCharacter == PLAYER)
    {
        graphics->drawImage(mPlayer, 5, 8);
    }
...
void Dialog::setText(const std::string &text)
{
    ...
    else if (text.size() > 2 && text.at(0) == 'O')
    {
        mCharacter = OLOF;
        mText = text.substr(2, text.size() - 2);
    }
    else if (text.size() > 2 && text.at(0) == 'P')
    {
        mCharacter = PLAYER;
        mText = text.substr(2, text.size() - 2);
    }
...
{
    if (keyEvent.getKey().getValue() == gcn::Key::LeftAlt
