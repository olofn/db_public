...
void Battle::onLogic()
{
	...
	if(myBattleState != State_PreGameOver) {
		...
		bool allEnemyCharactersDead = true;
		for(std::vector<EnemyCharacter *>::iterator it = myEnemyCharacters.begin(); it != myEnemyCharacters.end(); it++) {
			if(!(*it)->isDead()) {
				allEnemyCharactersDead = false;
				break;
			}
		}
		if(allEnemyCharactersDead) {
			myCurrentFight = NULL;
		}

	} else {
		myPreGameOverFrameCounter++;
		if(myPreGameOverFrameCounter > 200) {
			if(!myGameOverScreen) {
...
void Battle::privUpdateActions()
{
	...
	while (myCurrentAction == NULL && myActionQueue.size() > 0) {
		...
		if (myCurrentAction->getPerformer() != 0 && !isOnBattlefield(myCurrentAction->getPerformer())) {
